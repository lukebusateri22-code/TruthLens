"""
ROBUST TRAINING - CORRECT ARCHITECTURE FOR FRONTEND
Uses OptimizedManipulationCNN to match webapp
...
# Import the correct model from your frontend
import sys
sys.path.append('models')
from manipulation_detector_final import OptimizedManipulationCNN
...
def main():
    log_message("="*60)
    log_message("ROBUST TRAINING - CORRECT ARCHITECTURE")
    log_message("Using OptimizedManipulationCNN (matches frontend)")
    ...
    # Model - Use the CORRECT architecture for frontend
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    log_message(f"✓ Device: {device}")
    
    model = OptimizedManipulationCNN()
    
    # Try to load weights from the EfficientNet model (if possible)
    log_message("\nChecking for existing weights...")
    try:
        # Try to load the best available weights
        if Path('best_manipulation_model_final.pth').exists():
            checkpoint = torch.load('best_manipulation_model_final.pth', map_location='cpu')
            if 'model_state_dict' in checkpoint:
                model.load_state_dict(checkpoint['model_state_dict'])
            else:
                model.load_state_dict(checkpoint)
            log_message("✓ Loaded best_manipulation_model_final.pth")
        elif Path('best_manipulation_incremental.pth').exists():
            # Try to load EfficientNet weights (might not work due to architecture mismatch)
            log_message("⚠️ EfficientNet weights may not be compatible")
            log_message("Starting from scratch with correct architecture")
        else:
    ...
    for epoch in range(start_epoch, EPOCHS):
        ...
        try:
            ...
            if val_acc > best_val_acc:
                ...
                torch.save(model.state_dict(), 'best_manipulation_fast.pth')  # Frontend expects this name
